...
configureHttpd()
{
  ...
  echo '<FilesMatch \.php$>
    SetHandler "proxy:unix:/run/php56-fpm/php56-fpm.sock|fcgi://localhost/"
</FilesMatch> >> /etc/httpd/conf/httpd.conf
  sed -i 's/;extension=mysqli.so/extension=mysqli.so/g' /etc/php56/php.ini
	systemctl enable httpd php56-fpm;
	systemctl restart httpd php56-fpm >/dev/null 2>&1
	sleep 2;
	systemctl status httpd php56-fpm >/dev/null 2>&1;
	ret=$?;
	if [ "$ret" != "0" ]
	then
		echo "Failed! ($ret)";
